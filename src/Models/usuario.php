...
// --- FUNÇÃO PROVAVELMENTE FALTANDO ---
function buscarUsuarioPorResetToken(mysqli $conexao, string $token): ?array
{
    $sql = "SELECT * FROM usuarios WHERE reset_token = ? AND reset_token_expires_at > NOW() LIMIT 1";
    $stmt = $conexao->prepare($sql);
    if ($stmt === false) {
      error_log('Erro na preparação do SQL: ' . $conexao->error);
      return null;
